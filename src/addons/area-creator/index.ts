...
export default class AreaCreator extends WorkBalanceAddon {
	...
	get infos(): AddonInfos {
		return { enabled: true, name: 'area creator' };
	}
	inject(empWorkDialog: HTMLElement): void {
		...
		// という面倒くさいことが必要なくなった。レイアウトが変わり
		// [[ジョブ追加]]  [[登録] [キャンセル]]  [[設定]]  という横並びの3つのエリアから
		// [[ジョブ追加]]                         [[設定]]
		//                 [[登録] [キャンセル]]
		// と登録・キャンセルだけ1段下に下がった。
		// ので、疑似fit-contentは不要。ついでに使っていなかった[[ジョブ追加]]のエリアへの干渉をやめた

		const empWorkOption1 = empWorkDialog.querySelector<HTMLElement>('#empWorkOption1');
		if (!empWorkOption1) throw new Error('$("#empWorkOption1") not found.');
		const rightTd = empWorkOption1.parentElement;
		if (!rightTd) throw new Error('Ancestor element for $("#empWorkOk") not found.');
		UIs.create('div')
			.id(AreaCreator.WORK_DIALOG_FOOTER_RIGHT_ID)
			.style((css) => (css.display = 'inline-flex'))
			.append(...rightTd.childNodes)
